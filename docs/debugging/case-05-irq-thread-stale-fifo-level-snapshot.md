...
**Update, 2026-09-04 (V7):** characterized and largely closed. The gap was 3 register reads per sample (`REG_FIFO_LEVEL`+`REG_DATA_SEQ`+`REG_DATA_VAL`, `REG_FIFO_LEVEL` being this fix's own re-check cost), each dominated by the driver's `inter_frame_us` gap (500us at the time this case was written). A hardware sweep of that value found a non-monotonic relationship (200-300us was worse than either 500us or lower) and settled on a new default of 100us, reaching ~1000 samples/sec (matching the MCU's apparent production rate) with `kfifo_overflow`/sequence gaps mostly at 0 — see `private/session-log.md`'s 2026-09-04 fourth-round entry and `docs/debugging/case-06-*.md`'s updated "Next steps" for the full data.

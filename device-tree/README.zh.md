...
- 2026-09-04：V7 第一阶段——端到端延迟测量的第一步（Plan.md V7）。给
  驱动加了一个真正的硬中断上半部（`custom_acq_irq_hard()`），跟原来
  就有的线程下半部并存——之前只注册了线程处理函数（一直没有必须跑在
  硬中断上下文的逻辑），所以之前能拿到的最早时间戳只是"IRQ 线程被调
  度起来"，不是"中断真正到达"那一刻。硬中断部分现在会打一个
  `ktime_get_ns()` 时间戳，然后用 `IRQ_WAKE_THREAD` 交给线程部分；这
  个时间戳跟着每条 `struct custom_acq_sample`（新加 `irq_ts_ns` 字
  段，追加 8 字节，天然对齐不需要 padding）经过 kfifo 一路传到
  `/dev/acq0`。`device-service` 同步了这个结构体布局，用
  `std::chrono::steady_clock` 记自己收到的时间（跟内核 `ktime_get` 用
  的是同一个底层时钟，可以直接比较，不用换算纪元），还新加了一个可
  选的 `latency_log_path` 配置项（默认关闭——这是 V7 实验用的开关，不
  是生产特性），打开后会把每条 `(seq, irq_ts_ns, recv_ts_ns,
  latency_ns)` 记到一个 CSV 里，靠新写的 `LatencyLogger` 这个类实现。
  验证情况：驱动在这台 VM 的宿主机内核头文件下编译干净（是一次真正
  的外部内核模块编译，不只是语法检查）；新增的用户态代码通过了不依
  赖外部库的语法检查（`nlohmann-json`/`spdlog` 这台 VM 上除了 Yocto
  sysroot 之外没装，完整 CMake 编译这次没跑——真正的编译验证要靠
  `bitbake device-service`，这一轮还没重新跑）。已知且写清楚的局限：
  DATA_READY 是电平信号（case-03/case-05 讲过），一次硬中断触发可能
  在线程一次排空循环里搬走好几条 MCU 数据，这几条数据会共享同一个
  `irq_ts_ns`——所以这一步测的是"IRQ 到 userspace"这一段延迟，不是逐
  条样本真正的"MCU 产生到收到"延迟。更完整的那条链路（MCU 每产生一
  条数据就翻转一次专用测量引脚，配合逻辑分析仪采集）是 V7 后面阶段
  的工作，这次还没做。这一轮还没有重新编译烧录、上真机测试。

- 2026-09-04（更后段，真机验证）：SSH 到真机部署 V7 驱动/服务，找到
  了 case-05 吞吐瓶颈的真正根因——每条样本要做 3 次寄存器读
  （`REG_FIFO_LEVEL`+`REG_DATA_SEQ`+`REG_DATA_VAL`），不是原来以为
  的 2 次，所以两帧之间的 `inter_frame_us` 间隔对每条样本的开销影响
  很大。在真机上系统扫了一遍这个值（已经改成运行时模块参数），发现
  关系不是线性的——200-300us 反而是个比 500us（原默认）和更低的值
  都更容易丢数据的"坑"，降到 100us 以下才真正追上 MCU 的产生速度
  （吞吐从 ~520/s 冲到 ~1000/s，`kfifo_overflow`/`gap_count` 大多数
  时候是 0）。驱动的编译期默认值改成了 100us。这个改动顺带修好了上
  一条记录发现的延迟指标失真问题：吞吐追上之后，8 秒内抓到的样本里
  `irq_ts_ns` 有一万四千多个不同值（不再是几千条样本共享一个陈旧值），
  测出来的 IRQ-to-userspace 延迟合理（中位数约 971us，p99 约 976us，
  最大约 1163us）。过程中还发现并修好了自己的一个部署失误：
